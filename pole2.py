...
2. удалить все элементы с таким названием''')
        ...
        if k == 'показать весь список':
            for prod in spisok:
                print(prod)
        if k == 'редактировать элемент':
            print('''Выберите способ редактирования
1. через индекс
2. по имени''')
            while k == 1:
                t=int(input('Введите индекс элемента:'))
                e=input('Введите новое название элемента:')
                spisok[t]=e
            if k == 2:
                l=input('Введите название элемента, который хотите изменить:')
                p=input('Введите новое название элемента:')
                while l in spisok:
                    spisok[l]=p
                else:
